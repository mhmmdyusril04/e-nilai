  ...
  async handler(ctx, args) {
    ...
    if (pegawai.bidangId !== me.bidangId) {
      throw new ConvexError(
        "Anda hanya bisa menominasikan pegawai dari unit kerja Anda."
      );
    }
    ...
    if (existingNomination) {
      throw new ConvexError(
        "Pegawai ini sudah pernah dinominasikan untuk periode yang sama."
      );
    }
      ...
      nominasi = await ctx.db
        ...
        .filter((q) =>
          q.and(
            ...
            q.eq(q.field("status"), "dinominasikan")
          )
        )
        .order("desc")
        .collect();
...
export const getMyNominations = query({
  ...
  async handler(ctx) {
    ...
    const myNominations = nominations.filter((n) =>
      pegawaiIds.includes(n.pegawaiId)
    );

...
export default function KetuaBidangPage() {
  ...
  const [selectedPegawai, setSelectedPegawai] = useState<Doc<"pegawai"> | null>(
    null
  );
    ...
    <>
      ...
      <div className="container mx-auto p-4 md:p-8 space-y-8">
        <h1 className="text-4xl font-bold">
          {me?.role === "admin"
            ? "Tinjauan Pegawai (Admin)"
            : `Dashboard ${me?.bidang?.name ?? ""}`}
        </h1>
        ...
        {hasReachedNominationLimit && me?.role === "ketua_bidang" && (
          <div className="p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700">
            ...
            <p>
              Anda sudah menominasikan 2 pegawai. Anda tidak dapat menominasikan
              lagi.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {pegawaiList?.map((pegawai) => {
            ...
            const isButtonDisabled =
              me?.role === "admin" || isNominated || hasReachedNominationLimit;
